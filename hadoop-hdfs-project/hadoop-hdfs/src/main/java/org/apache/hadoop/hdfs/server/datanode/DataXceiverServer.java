 ...
 * DataXceiverServer是hadoop分布式分析系统hdfs的从节点dn上的一个后来工作线程
 * 它类似一个小型服务器，被用来接收数据读写请求，并且为每个请求创建一个工作线程进行请求响应
 * 他主要用于接收发送block
 */
class DataXceiverServer implements Runnable {
  ...
  public void run() {
    ...
    while (datanode.shouldRun && !datanode.shutdownForUpgrade) {
      try {
        //todo 通过accept接收客户端请求
        peer = peerServer.accept();

        // Make sure the xceiver count is not exceeded
        int curXceiverCount = datanode.getXceiverCount();
        //如果maxXceiverCount超出日志会抛出异常
        if (curXceiverCount > maxXceiverCount) {
          throw new IOException("Xceiver count " + curXceiverCount
              + " exceeds the limit of concurrent xcievers: "
              + maxXceiverCount);
        }
        //客户端每次发送一个block块的数据，这里会开启一个DataXceiver线程去处理
        //todo 进入DataXceiver类中，查看run方法
