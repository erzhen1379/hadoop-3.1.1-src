...
public class DatanodeManager {
    ...
    public DatanodeDescriptor getDatanode(final String datanodeUuid) {
        ...
        synchronized (this) {
            //key,value  : datanodeUuid,DatanodeDescriptor
    ...
    boolean isDatanodeDead(DatanodeDescriptor node) {
        //如果心跳10分钟30秒没有更新，那说明dn已经dead了
        return (node.getLastUpdateMonotonic() <
                (monotonicNow() - heartbeatExpireInterval));
    }

    /** Add a datanode.
     * 注册daanode ，往一堆数据结构里面添加信息
     * */
    void addDatanode(final DatanodeDescriptor node) {
        ...
        //如果以上内存数据结构里面的数据添加好了以后，就注册完了
        LOG.debug("DatanodeManager 端（nn）注册dn信息为" + node);
        ...
        for (DatanodeDescriptor node : copy.values()) {
            ...
            if (!hostConfigManager.isIncluded(node)) {
                ...
                if (node.maintenanceNotExpired(maintenanceExpireTimeInMS)) {
                    ...
                    //在exclude文件中，则直接调用startDecommission（）开始撤销操作
                    datanodeAdminManager.startDecommission(node);
                } else {
                    //不在exclude文件中，则停止撤销操作
    ...
    public DatanodeCommand[] handleHeartbeat(DatanodeRegistration nodeReg,
        ...
        try {
            //如果能够获取这个dn信息说明一起就注册过
            //但是如果是第一次注册，那么datanodemap里面是没有的信息
