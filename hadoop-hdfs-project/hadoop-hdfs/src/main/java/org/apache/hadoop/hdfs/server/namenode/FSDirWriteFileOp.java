...
class FSDirWriteFileOp {
  ...
  static HdfsFileStatus startFile(
    ...
    //判断要创建的文件路径是否存在，如果存在，则进入if语句中
    if (iip.getLastINode() != null) {
      //如果可以覆盖，则进入if，否则进入else
      if (overwrite) {
    ...
    //maxFsObject的值默认0，表示不限制
    fsn.checkFsObjectLimit();
    INodeFile newNode = null;
    //创建的文件路径中不存在的父目录
    // /a/b/c.txt 如果父目录a已经存在，目录b不存在，则创建目录b
    INodesInPath parent =
        FSDirMkdirOp.createAncestorDirectories(fsd, iip, permissions);
    if (parent != null) {
      //在hdfs中创建文件，真正执行创建文件的操作，进入addFile方法
    ...
    //添加文件租约
    //租约：在hdfs中，客户端往某个文件中写数据的时候，为了保持数据的一致性，此时客户端不允许想此文件中写数据。
    //hdfs 通过租约，租约是客户端的一个写文件操作的临时许可证，无此证件这江不允许操作此文件。
  ...
  private static INodesInPath addFile(
    ...
    try {
      ...
      //block块的布局方式，连续布局，or 条形布局
      final BlockType blockType = isStriped ?
          BlockType.STRIPED : BlockType.CONTIGUOUS;
      //计算副本因子数，如果是连续布局的情况下返回replication 否则返回null
      final Short replicationFactor = (!isStriped ? replication : null);
      //获取纠删码的id，如果是条形布局情况下返回ecPolicy.getId(),否则返回null
