...
class BPServiceActor implements Runnable {
  ...
  final InetSocketAddress nnAddr; //对应nn地址
  HAServiceState state;           //nn状态
  ...
  Thread bpThread;                    //当前工作的线程，是BPServiceActor主逻辑的执行线程
  DatanodeProtocolClientSideTranslatorPB bpNamenode;   //向nn发送rpc请求的代理，dn向nn发起dp调用都是通过这个引用进行的。
  //当前状态
  enum RunningState {
  ...
  private final DataNode dn;  //记录dn的配置，
  private final DNConf dnConf;   //记录dn的配置，
  ...
  HeartbeatResponse sendHeartBeat(boolean requestBlockReportLease)
    ...
    //发送的具体信息org.apache.hadoop.hdfs.server.namenode.NameNodeRpcServer.sendHeartbeat
  ...
  private void offerService() throws Exception {
    ...
    while (shouldRun()) {
      try {
        ...
        //每3秒进行一次nextHeartbeatTime - startTime
  ...
  void register(NamespaceInfo nsInfo) throws IOException {
    ...
    //构造dn注册请求，这里storageInfostorage以及dnuuid 已经通过上一阶段的握手获得，并且持久化到磁盘上了
    DatanodeRegistration newBpRegistration = bpos.createRegistration();

    LOG.info(this + " beginning handshake with NN");
    //调用registerDatanode（）注册dn
    //查看对应注册方法
    while (shouldRun()) {
      try {
        // Use returned registration from namenode with updated fields
        //调用服务端的registerDatanode方法
        //org.apache.hadoop.hdfs.server.namenode.NameNodeRpcServer.registerDatanode
        newBpRegistration = bpNamenode.registerDatanode(newBpRegistration);
        //如果执行到这说明注册已经完成
        LOG.debug("BPserviceActor开始向nn注册的就行信息：");
    ...
    //调用registrationSucceeded（）方法确认，namenodeId clusterId与其他nn返回信息一致，并且确认dnuuid与dn本地存储一致
  ...
  public void run() {
    ...
    try {
      while (true) { //方法比较重要，尽量多次注册
        // init stuff
        //初始化
        try {
          // setup storage
          //与namenode握手，初始化dn存储并且注册（重要）
          connectToNNAndHandshake();
          break;
        } catch (IOException ioe) {
          // 握手，存储恢复或者注册操作出现异常，将运行状态转为INIT_FAILED
      ...
      //线程的主循环，循环调用offerService（）方法向nn发送心跳，块汇报，增量快汇报以及缓存
      //块汇报，并执行nm返回的名字节点指令
      while (shouldRun()) {
        try {
          offerService();
          ...
          //收到异常一不理会，继续循环到BPServiceActor停止或者dn停止
