...
public abstract class Receiver implements DataTransferProtocol {
  ...
  protected final void processOp(Op op) throws IOException {
    switch(op) {
    case READ_BLOCK:
      opReadBlock();  //匹配到这个方法，读取
      break;
      //匹配到这个case ，进入writeBlock方法
    case WRITE_BLOCK:
      opWriteBlock(in);
      break;
    case REPLACE_BLOCK:
  ...
  private void opReadBlock() throws IOException {
    ...
    try {
      //todo 反序列化参数，然后调用子类DataXceiver的readBlock（） 方法执行读操作
  ...
  private void opWriteBlock(DataInputStream in) throws IOException {
    ...
    TraceScope traceScope = continueTraceSpan(proto.getHeader(),
        proto.getClass().getSimpleName());
    try {
      //进入dataxceiver的writeBlock方法 进去看这个org.apache.hadoop.hdfs.server.datanode.DataXceiver.writeBlock
