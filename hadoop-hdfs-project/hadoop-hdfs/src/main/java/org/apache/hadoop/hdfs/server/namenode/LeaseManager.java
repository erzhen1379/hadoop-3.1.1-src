...
public class LeaseManager {
   ...
   * calling this method.
   * 该方法遍历所有租约并且计算块数，fsnamesystem读锁不要调用该方法
   */
  synchronized long getNumUnderConstructionBlocks() {
    assert this.fsnamesystem.hasReadLock() : "The FSNamesystem read lock wasn't"
      + "acquired before counting under construction blocks";
    long numUCBlocks = 0;
    for (Long id : getINodeIdWithLeases()) {  //1.遍历所有有租约
      INode inode = fsnamesystem.getFSDirectory().getInode(id);  //遍历所有租约（可以看成hdfs的一个目录）
      if (inode == null) {
