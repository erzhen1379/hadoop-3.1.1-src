...
class BlockReceiver implements Closeable {
    ...
    try{
      ...
      if (isDatanode) { //replication or move
        ...
        case PIPELINE_SETUP_CREATE:
          ...
          break;
          //向rbw目录中写数据
          //解释：HDFS 在本地磁盘会产生2个目录，finalized目录和rbw目录
          //这2个目录都是用于实际存储hdfs block的数据，里面包含很多block_xx文件以及响应的.meta文件，
          //rbw 是replica being written 的意思，该目录用于存储当前正在写的数据
          //todo 当数据写完后，会改名字，摈弃放到finalized目录中
  ...
  private int receivePacket() throws IOException {
    ...
    //todo 1.blockReceeiver接收数据
    if (responder != null && !syncBlock && !shouldVerifyChecksum()) {
      //todo 把packet写入到当前dtanode的ackQueue中,进入当前方法
    ...
    //todo 2.把当前的这个packet发送给下游节点（数据管道里面的）
  ...
  void receiveBlock(
    ...
    try {
      if (isClient && !isTransfer) {
        //todo 启动PacketResponder()线程处理确认包的接收和转发
        responder = new Daemon(datanode.threadGroup, 
            new PacketResponder(replyOut, mirrIn, downstreams));
        responder.start(); // start thread to processes responses
      }
      //todo 循环调用receivePacket()接收并且转发数据块中的所有数据包
      while (receivePacket() >= 0) { /* Receive until the last packet */ }

      // wait for all outstanding packet responses. And then
      // indicate responder to gracefully shutdown.
      // Mark that responder has been closed for future processing
      //完成数据块的写入操作，结束PacketResponder
      if (responder != null) {
  ...
  class PacketResponder implements Runnable, Closeable {   
        ...
        try {
          ...
          try {
            ...
            if (Thread.interrupted()) {
              ...
              // notify client of the error
              // and wait for the client to shut down the pipeline
              //如果从下游节点读取数据抛出异常，则将mirrorError设置为true
              mirrorError = true;
          ...
          if (Thread.interrupted() || isInterrupted) {
             ...
             *对于线程中断，则将running设置为false，停止当前运行线程
             */
          ...
          if (lastPacketInBlock) {
            ...
            //todo 如果数据包响应是数据块的最后一个数据包响应，则提交这个数据块并关闭此block 文件（此处记录一个开始时间）
            finalizeBlock(startTime);
          }

          Status myStatus = pkt != null ? pkt.ackStatus : Status.SUCCESS;
          //todo 直接往上游节点发送处理结果
          //复制下游节点的数据包响应，加入当前数据节点的状态，并且发送给上游节点
          //todo 进入当前方法
          ...
          if (pkt != null) {
            ...
            //todo 如果下游数据处理成功，从当前datanode就会从ackqueue中移除
            removeAckHead();
          }
        } catch (IOException e) {
    ...
    private void sendAckUpstreamUnprotected(PipelineAck ack, long seqno,
      ...
      if (ack == null) {
        ...
        //如果下游节点发送的是oob消息，则保留下游节点信息
        replies = new int[] { myHeader };
      } else if (mirrorError) { // ack read error如果是当前节点从下游节点读取响应消息异常
        ...
        replies[0] = myHeader;  //将当前数据状态放入响应中
        ...
        //如果上游节点检测到和校验和错误，那么当前节点发送的数据有错误
        //则停止Blockreceiver 和packetRespinder线程
