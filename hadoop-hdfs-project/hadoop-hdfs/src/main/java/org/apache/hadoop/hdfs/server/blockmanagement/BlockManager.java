...
public class BlockManager implements BlockStatsMXBean {
  ...
  throws IOException {
    ...
    int numCurrentReplica = numLiveReplicas + pendingNum;
     //当前数据块处于提交状态，并且数据块的副本已经满足最小副本的要求才会被调用
    if(storedBlock.getBlockUCState() == BlockUCState.COMMITTED &&
    ...
    //如果该数据块的有效副本数量已经超过期望，调用invalidateCorruptReplicas()方法将数据块从dn上删除
  ...
  public void removeStoredBlock(BlockInfo storedBlock, DatanodeDescriptor node) {
    ...
    assert (namesystem.hasWriteLock());
    {
      if (storedBlock == null || !blocksMap.removeNode(storedBlock, node)) {
       ...
       //这次删除操作有可能是数据节点发生错误，但是该数据依然有效
      //在这种情况下数据块的副本数量可能不足，更新neededReplications队列
      ...
      //已经从该数据节点上删除副本，更新exceessReplicateMap队列
      excessRedundancyMap.remove(node, storedBlock);
      //已经从该节点上删除副本，更新corruptReplicas()队列
