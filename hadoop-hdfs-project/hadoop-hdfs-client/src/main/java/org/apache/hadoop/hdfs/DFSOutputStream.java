...
public class DFSOutputStream extends FSOutputSummer
   ...
   * @return
   * @throws IOException
   * 1.通过hdfs client.namenode.create 在hdfs的目录中创建一个文件
   * 2.通过new DFSOutputStream创建一个该文件输出流对象，在DFSOutputStream构造方法中，初始化了用户数据处理的DFSOutStream类的内部类DataStreamer,用于启动DataStreamer线程，接收客户端写入数据的请求。
   * 3.DataStreamer是一个线程，他的启动是通过DFSOutputStream的start方法来启动的。
   */
  static DFSOutputStream newStreamForCreate(DFSClient dfsClient, String src,
    ...
    try (TraceScope ignored =
      ...
      while (shouldRetry) {
        shouldRetry = false;
        try {
          //HDFS 客户端向namenode发送rpc请求，最终调用naemnoderpc中的create方法
          //通过create方法可以在hdfs的目录上创建一个文件
          //todo 1.org.apache.hadoop.hdfs.server.namenode.NameNodeRpcServer.create
      ...
      if(stat.getErasureCodingPolicy() != null) {
        ...
        //构造一个DFSOutputStream输出流对象
        //todo 2.DFSOutputStream构造方法内部会计算packet，chunk的大小，初始化datastreamer线程对象
  ...
  protected synchronized void writeChunk(byte[] b, int offset, int len,
    ...
    //todo 向packet中写chunk的校验和（checksum） 4byte
    currentPacket.writeChecksum(checksum, ckoff, cklen);
    //todo 向packet里面写一个chunk  512 byte
    currentPacket.writeData(b, offset, len);
    //todo 累计以供有多少个chunk  -》 packet 如果写满了一个127chunk，，那就是一个完整的packet
    currentPacket.incNumChunks();
    //进入这个方法
    getStreamer().incBytesCurBlock(len);
    //判断当前packet是否满了，如果不满，则继续循环执行上一层的for循环
    // If packet is full, enqueue it for transmission
    //todo 2个条件
    //1.如果写满了一个packet(127 chunk) =packet
    //2.一个文件块写满了 block 128M，
    if (currentPacket.getNumChunks() == currentPacket.getMaxChunks() ||
        getStreamer().getBytesCurBlock() == blockSize) {
      //todo 如果packet写满了，则江packet写入队列data queue中，进入enqueueCurrentPacketFull方法
  ...
  */
  ...
  static LocatedBlock addBlock(DatanodeInfo[] excludedNodes,
    ...
    while (true) {
      try {
        //客户端通过rpc机制和namenode进行通信
        //进入namenoderpcserver的addBlock方法
        LOG.debug("客户端通过rpc机制和namenode进行通信，申请分配一个新的数据块");

        LocatedBlock locatedBlock = dfsClient.namenode.addBlock(src, dfsClient.clientName, prevBlock,
                excludedNodes, fileId, favoredNodes, allocFlags);
        LOG.debug("查看nn返回信息org.apache.hadoop.hdfs.DFSOutputStream.addBlock"+locatedBlock.toString());
        return locatedBlock;
